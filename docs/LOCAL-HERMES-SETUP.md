...
are disabled in this profile. Install `colonyai[graph,vectors]==1.1.2` only when
...
```sh
python -m pip install --upgrade "colonyai[hermes]==1.1.2" "colony-hermes==1.1.2"
colony init --non-interactive --hermes-home "$HOME/.hermes-orion" --refresh-adapter
```

Replace `1.1.2` with the release you are selecting. A Hermes interpreter with
