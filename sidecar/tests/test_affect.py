...
class TestAffectCreateEvent:
    ...
    def test_valence_clamped(self, store):
        result = store.create_event(contact_id="marc", valence=2.0, source="explicit")
        assert result["valence"] == 1.0

    def test_valence_negative_clamped(self, store):
        result = store.create_event(contact_id="marc", valence=-3.0, source="explicit")
        assert result["valence"] == -1.0

    def test_arousal_clamped(self, store):
        result = store.create_event(contact_id="marc", valence=0.0, arousal=1.5, source="explicit")
        assert result["arousal"] == 1.0

    def test_custom_timestamp(self, store):
        ts = "2026-01-01T00:00:00+00:00"
        result = store.create_event(contact_id="marc", valence=0.3, source="explicit", timestamp=ts)
        assert result["timestamp"] == ts


class TestAffectGetEvent:
    def test_get_existing(self, store):
        created = store.create_event(contact_id="marc", valence=0.5, source="explicit")
...
class TestAffectListEvents:
    def test_list_all(self, store):
        store.create_event(contact_id="marc", valence=0.5, source="explicit")
    ...
    def test_list_by_contact(self, store):
        store.create_event(contact_id="marc", valence=0.5, source="explicit")
        store.create_event(contact_id="alice", valence=-0.3, source="inferred")
        events = store.list_events(contact_id="marc")
        assert len(events) == 1
        assert events[0]["contact_id"] == "marc"

    def test_list_by_source(self, store):
        store.create_event(contact_id="marc", valence=0.5, source="explicit")
        store.create_event(contact_id="marc", valence=0.3, source="inferred")
    ...
    def test_list_pagination(self, store):
        for i in range(5):
            store.create_event(contact_id="marc", valence=0.1 * i, source="explicit")
...
class TestAffectDeleteEvent:
    ...
    def test_delete_recomputes_state(self, store):
        e1 = store.create_event(contact_id="marc", valence=0.8, source="explicit")
        store.create_event(contact_id="marc", valence=0.2, source="explicit")
        store.delete_event(e1["id"])
        state = store.get_state("marc")
...
class TestAffectState:
    ...
    def test_state_multiple_events(self, store):
        store.create_event(contact_id="marc", valence=-0.3, source="explicit")
        store.create_event(contact_id="marc", valence=0.5, source="explicit")
        store.create_event(contact_id="marc", valence=0.8, source="explicit")
        state = store.get_state("marc")
    ...
    def test_stable_trend(self, store):
        store.create_event(contact_id="marc", valence=0.3, source="explicit")
        store.create_event(contact_id="marc", valence=0.35, source="explicit")
        state = store.get_state("marc")
        assert state["trend"] == "stable"
...
class TestAffectDetection:
    def test_negative_spike_detected(self, store):
        store.create_event(contact_id="marc", valence=-0.7, source="explicit")
        assert store.detect_negative_spike("marc") is True

    def test_no_negative_spike(self, store):
        store.create_event(contact_id="marc", valence=0.3, source="explicit")
        assert store.detect_negative_spike("marc") is False
    ...
    def test_sustained_decline(self, store):
        ...
        store.create_event(contact_id="marc", valence=0.3, source="explicit")
        store.create_event(contact_id="marc", valence=-0.2, source="explicit")
        assert store.detect_sustained_decline("marc") is True

    def test_no_sustained_decline(self, store):
        store.create_event(contact_id="marc", valence=0.5, source="explicit")
        assert store.detect_sustained_decline("marc") is False
