...
from colony_sidecar.goals.models import GoalPriority, GoalSource
...
def _parse_turn_content(content: str) -> List[ConversationMessage]:
    ...
    turns_sync stores combined summaries as:
        User: {text}
        Assistant: {text}
...
class ConversationSynthesisTask:
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        # 2. Filter out already-processed memories (by ID or timestamp)
        watermark = self.state.last_processed_at
        new_memories = []
        for mem in memories:
            mem_time = mem.get("created_at")
            if watermark and mem_time and mem_time <= watermark:
                continue
        ...
        for mem in new_memories:
            ...
            # Only keep candidates from user messages (assistant messages
            # shouldn't generate goals for the owner)
            user_candidates = [c for c in candidates if any(
                m.role == "user" for m in messages if m.content in c.source_messages or m.content == c.description
            )]
        ...
        # 5. Create goals via GoalEngine
        goals_created = 0
        for cand in unique_candidates:
            try:
                created = await goals.propose_goal(
                    title=cand.title,
        ...
        # 6. Update watermark and state
        newest_time = max(
            (m.get("created_at", "") for m in new_memories),
            default=None,
        )
        if newest_time:
